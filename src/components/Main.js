...
export default function Main({
  ...
  const movementHandler = direction => {
    axiosWithAuth()
      .post('https://pitch-black.herokuapp.com/api/adv/move/', {
        direction: direction
      })
      .then(res => {
        console.log(res);
  ...
  const moveHandler = direction => {
    if (direction === 'up') {
      if (
        map.getTile(playerLocation.x, playerLocation.y - 1) === 0 ||
        map.getTile(playerLocation.x, playerLocation.y - 1) === undefined
      ) {
      } else {
        movementHandler('d');
      }
      ...
      if (
        map.getTile(playerLocation.x - 1, playerLocation.y) === 0 ||
        playerLocation.x - 1 < 0
      ) {
      } else {
        movementHandler('l');
      }
  ...
  const grabItem = itemName => {
    axiosWithAuth()
      .post('https://pitch-black.herokuapp.com/api/adv/get-item/', {
        item: itemName
      })
      .then(res => {
        console.log(res);
  ...
  const dropItem = itemName => {
    axiosWithAuth()
      .post('https://pitch-black.herokuapp.com/api/adv/drop-item/', {
        item: itemName
      })
      .then(res => {
        console.log(res);
    ...
    return (
      <Container id='main'>
        <Typography
          ...
          style={{
            textShadow:
              '0px 0px 5px #ff1003, 0px 0px 10px #ff1003, 0px 0px 25px #ff1003',
            fontWeight: 'bold'
          }}
        >
          Pitch Black
        ...
        <Typography
          ...
          style={{
            marginBottom: '50px',
            textShadow:
              '0px 0px 2px #ff1003, 0px 0px 5px #ff1003, 0px 0px 10px #ff1003',
            fontWeight: 'bold'
          }}
        >
        ...
        <Grid container justify='center' spacing={3}>
          <Grid item>
            <div
              className='ui-item'
              style={{ minWidth: '750px', minHeight: '750px' }}
            >
              {isLoaded ? (
                <Dungeon
                  map={map}
                  ...
                  {false ? (
                    <>
                      ...
                      <input
                        ...
                        style={{ width: 314, borderRadius: 5 }}
                      />
                    </>
                  ) : (
